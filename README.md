...
To draw a circle that moves with our mouse position, we need to declare a function `drawCircle`. This function is gonna have two parameters (x, y), that refers to the coordinate of the position where is gonna draw on the canvas. Inside the `body`, which contains the statements that are to be executed, we call the `arc()` method that is used to create circles. Next step is to declare a `clear` function with the `clearRect` method that erase the canvas every time we are moving the mouse. But to actually draw the circle, we have to create a function `animate`. Inside the body, we call the `requestAnimationFrame` method. This method tells the browser that an animation is gonna to be happen, with a number of `callbacks` that is usually 60 times per second. Inside the `animate` function we call now our previous functions and add to our `drawCircle` two arguments of the mouse object `drawCircle(mouse.x, mouse.y)`, this gives our circle the position where the mouse is moving. 
